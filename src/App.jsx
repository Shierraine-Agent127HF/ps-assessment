...
import { Brain, Eye, Calendar, MessageSquare, Lightbulb, FileText } from "lucide-react"

// ═══════════════ DATA ═══════════════════════════════════════════
const SECS = [
  { id:"ct", title:"Critical Thinking", color:"#4ec9b0", Icon:Brain, qs:[
  ...
  { id:"ad", title:"Attention to Detail", color:"#dcdcaa", Icon:Eye, qs:[
  ...
  { id:"po", title:"Planning & Org", color:"#9cdcfe", Icon:Calendar, qs:[
  ...
  { id:"cm", title:"Communication", color:"#c586c0", Icon:MessageSquare, qs:[
  ...
  { id:"ps", title:"Problem-Solving", color:"#f14c4c", Icon:Lightbulb, qs:[
  ...
  { id:"es", title:"Essay Prompts", color:"#6a9955", Icon:FileText, qs:[
...
export default function App() {
  ...
  const navBtn={padding:"6px 16px",fontSize:12,border:`0.5px solid ${C.bStr}`,borderRadius:6,cursor:"pointer",background:"transparent",color:C.textSec,fontFamily:"var(--font-sans)"}
  const subBtn={padding:"8px 20px",fontSize:13,border:"none",borderRadius:6,cursor:"pointer",background:"var(--fill-accent)",color:"var(--on-accent)",fontFamily:"var(--font-sans)",fontWeight:500}
  const optSt=(sel,correct,wrong)=>({display:"flex",gap:10,padding:"9px 12px",borderRadius:6,cursor:phase==="done"?"default":"pointer",marginBottom:5,fontFamily:"var(--font-sans)",fontSize:13,lineHeight:1.5,alignItems:"flex-start",border:`0.5px solid ${wrong?C.bdDanger:correct?C.bdSuccess:sel?C.bdAccent:C.border}`,background:wrong?C.bgDanger:correct?C.bgSuccess:sel?C.bgAccent:"transparent",color:wrong?C.danger:correct?C.success:sel?C.accent:C.textSec})

  // ── Welcome ──
  if(phase==="welcome"||phase==="validating") return (
    <div style={{...root,alignItems:"center",justifyContent:"center",padding:"0 20px"}}>
  ...
  // ── Results ──
  const ResultsView=()=>(
    <div style={{padding:"24px 28px",overflowY:"auto",flex:1,fontFamily:"var(--font-sans)"}}>
  ...
  const titleBtn=(active)=>({padding:"3px 10px",fontSize:11,borderRadius:4,border:`0.5px solid ${active?C.bdAccent:C.border}`,background:active?C.bgAccent:"transparent",color:active?C.accent:C.textSec,cursor:"pointer"})
  const tabBtn=(active,col)=>({padding:"6px 14px",fontSize:11,cursor:"pointer",borderRight:`0.5px solid ${C.border}`,whiteSpace:"nowrap",fontFamily:"var(--font-sans)",borderTop:active?`2px solid ${col}`:"2px solid transparent",background:active?C.bg2:"transparent",color:active?C.text:C.textMute,flexShrink:0})

  return (
    <div style={root}>
      {/* Title bar */}
      <div style={{height:32,background:C.bg1,borderBottom:`0.5px solid ${C.border}`,display:"flex",alignItems:"center",justifyContent:"space-between",padding:"0 14px",flexShrink:0}}>
        ...
        <div style={{display:"flex",gap:6}}>
          {["quiz","results"].map(v=><button key={v} style={titleBtn(panel===v)} onClick={()=>setPanel(v)}>{v==="quiz"?"Questions":"Results"}</button>)}
        </div>
      </div>
      ...
      <div style={{display:"flex",flex:1,overflow:"hidden"}}>
        ...
        <div style={{flex:1,display:"flex",flexDirection:"column",overflow:"hidden"}}>
          ...
          {panel==="quiz"&&(
            <div ref={editorRef} style={{flex:1,overflowY:"auto",padding:"22px 28px",background:C.bg2}}>
